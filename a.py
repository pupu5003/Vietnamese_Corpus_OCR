...
data_dict = {
    # id, sentences per page, wide_crop, margin_x, width_ths_pass, margin_short_sentence, margin_long_sentence
    10693040: (12, 400, 250, 20, 457, 300),      
    10693454: (11, 400, 200, 10),       
    10695896: (12, 200, 190, 10),     
    10696073: (10, 400),       
    10709453: (15, 7)       
}
...
output_folder = f'image_crop_sentence/{pdf_id}'
...
margin_short_sentence = 457
margin_long_sentence = 300
def delete_box(boxes):
    ...
    while i < h:
        if abs(boxes[i][0][0][1] - boxes[i][0][2][1]) >= 150:
            boxes.pop(i)
            h -= 1
        elif abs(boxes[i][0][0][0] - boxes[i][0][1][0]) <= 300:
            boxes.pop(i)
            h -= 1
        else:
            i += 1
...
for filename in os.listdir(input_folder):
    if filename.lower().endswith(('.png', '.jpg', '.jpeg')): 
        ...
        results = reader.readtext('enhanced_image.png', height_ths=1, slope_ths=0.9, width_ths=width_ths_pass)
        ...
        for box in merged_boxes:
            ...
            # Crop the image using the adjusted top_left and bottom_right
            if top_left[0] < margin_x:
                cropped_image = image[top_left[1]:bottom_right[1], margin_x:bottom_right[0]]
            else:
                cropped_image = image[top_left[1]:bottom_right[1], min(margin_x, top_left[0]):bottom_right[0]]
            ...
            if cropped_image is None or cropped_image.size == 0 or abs(cropped_image.shape[1]) <= 300: # x<=700
                ...
                # cropped_image_path = os.path.join(file_output_folder, f'cropped_setence_{index:03}.png')
