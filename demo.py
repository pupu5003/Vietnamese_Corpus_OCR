...
for filename in os.listdir(input_folder):
    if filename.lower().endswith(('.png', '.jpg', '.jpeg')): 
        ...
        for box in merged_boxes:
            ...
            if abs(top_left[0]) <= margin_x:
                top_left = (margin_x, top_left[1])

            cropped_image = image[top_left[1]-10:bottom_right[1]+10, max(margin_x, top_left[0])-20:bottom_right[0]]
            ...
            if cropped_image is None or cropped_image.size == 0 or abs(cropped_image.shape[1]) <= crop_image_wide: # x<=700
                ...
                if (index % 2 == 0 and cnt != 8) or (index % 2 != 0 and cnt != 6):
                    ...
                    if abs(word[0][0][0][0]) > 150:
                        x_start = 0
                        x_end = word[0][0][0][0]-10
                        # y_start = word[0][0][0][1]
                        y_start = 0
                    ...
                    for j in range(1, len(word)):
                        ...
                        if current_distance > 150:
                            ...
                            x_start = word[j-1][0][1][0]
                            x_end = word[j][0][0][0]
                            # y_start = min(word[j-1][0][1][1], word[j][0][0][1])
                            y_start = 0
                            y_end = max(word[j-1][0][3][1], word[j][0][2][1])
                    ...
                    # Last word
                    if abs(word[len(word)-1][0][1][0] - cropped_image_1.shape[1]) > 150:
